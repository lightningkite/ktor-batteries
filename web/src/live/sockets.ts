...
import { delay, distinctUntilChanged, map, publishReplay, refCount, retryWhen, switchMap, tap, timeout } from 'rxjs/operators'
            ...
            return ((): Observable<WebSocketInterface> => {
                if ((!it)) { return NEVER } else {
                    ...
                    return (runOrNull(get_overrideWebSocketProvider(), _ => _(url)) ?? HttpClient.INSTANCE.webSocket(url))
                        ...
                        const pingMessages: Observable<WebSocketInterface> = interval(5000)
